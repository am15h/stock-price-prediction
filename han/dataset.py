...
class SnP500Dataset:
	def __init__(self, flags):
		...
		self.stock_dict, self.stock_date_dict, self.down_bound, self.up_bound = \
	...
	def load_tweets(self, input_dir):
		...
		lblst = ["Positive", "Negative", "Neutral"]
		for root, subdirs, files in os.walk(input_dir):
			...
			for filename in files:
				...
				with open(file_path, 'r', encoding='utf-8') as f:
					for line in f:
						...
						text = line_dict['text']
						# tweet = ' '.join(text)
						# result: dict = openai.Classification.create(
						# 	query=tweet,
						# 	search_model="ada",
						# 	model="curie",
						# 	examples=exlst,
		...
		# for stock_name in self.stock_name_set:
		# 	stock_history = self.stock_dict[stock_name]
		#
		# 	for i in range(len(stock_history)):
		# 		stock_date = stock_history[i][0]
		# 		stock_key = stock_name + '\t' + str(stock_date)
		#
		# 		if not date_tweets.__contains__(stock_key):
		# 			continue
		#
		# 		for j in range(len(date_tweets[stock_key])):
		# 			for k in [1, 2, 3, 4, 5, 6]:
		# 				date_tweets[stock_key][j].append(stock_history[i][k])
	...
	def load_stock_history(self):
			...
			with open(filepath, 'r', encoding='utf-8') as f:
				...
				for l in reversed(list(f)):
					...
					stock_dict[stock_name].append(
						[stock_date, price_diff_percent, open_price, high_price, low_price, close_price, volume]
					)
					stock_key = stock_name + '\t' + str(stock_date)
					if stock_key not in stock_date_dict:
						stock_date_dict[stock_key] = list()
					stock_date_dict[stock_key] \
						= [price_diff_percent, open_price, high_price, low_price, close_price, volume]
					num_trading_days += 1
		...
		return stock_dict, stock_date_dict, down_bound, up_bound
				...
				for j in [5, 4, 3, 2, 1]:
					...
					if stock_key in self.date_tweets:
						...
						for w_idxes in tweets:
							w_idx_str = '\t'.join([str(widx) for widx in w_idxes])
							params_str = '\t'.join([str(p) for p in [float(0.0)]*6])
							if self.stock_date_dict.__contains__(stock_key):
								params_str = '\t'.join([str(p) for p in self.stock_date_dict[stock_key]])
							ex_1.append('~~'.join([w_idx_str, params_str]))
	...
	def get_dataset(self, batch_size, max_date_len, max_news_len):
		...
		# ((days, params, day_lens, news_lens), labels))
	...
	def _get_idxes_len(self, x_texts, max_date_len, max_news_len):
		...
		for d in x_texts:
			...
			if len(d) > 0:
				...
				for n in news_list:
					if len(n) > 0:
						word_idxes = [float(w) for w in (n.split('~~')[0]).split('\t')]
						params = [float(p) for p in (n.split('~~')[1]).split('\t')]
						# slicing
						if len(word_idxes) > max_news_len:
							word_idxes = word_idxes[:max_news_len]

						for i in range(max_news_len - len(params)):
							params.append(float(0.0))
		...
		padded = np.zeros((x.shape[0], x.shape[1], x.shape[2] + 20), dtype=np.float32)
		padded[:x_params.shape[0], :x_params.shape[1], :x_params.shape[2]] = x_params

		x_date_len = np.array(x_date_len, dtype=np.int32)
		x_date_news_len = np.array(x_date_news_len, dtype=np.int32)

		# x, params, x_date_len, x_date_news_len

		return x, padded, x_date_len, x_date_news_len
